...
export function install(P) {
  P._spawnBullet = function (x, z, dx, dz, o) {
    this.bullets.push({ x, z, y: o.y, dx, dz, life: o.life || .55, dmg: o.dmg || 0, pierce: o.pierce || 0, ghost: o.ghost, ally: o.ally, tur: o.tur, own: o.own || 0, band: o.band || 0 });
  };
  ...
  P._autoCombat = function (p, dt, mine) {
    ...
    if (best) { p.fireT = 1 / Math.min(7.5, p.frate); this._fire(p, best.x, best.z, mine); } // hard fire-rate ceiling
  };
  ...
  P._turretSim = function (dt) {
    ...
    for (let i = 0; i < N * N; i++) {
      ...
      if (best) {
        this._turCd[i] = .3; const q = this._ownerOf(i); const band = this._turBand(q);
        const a = Math.atan2(best.z - z, best.x - x);
  ...
  P._bulletSim = function (dt) {
    ...
    for (const b of this.bullets) {
      ...
      for (const e of this.enemies.values()) {
        ...
        if (dist2(b.x, b.z, e.x, e.z) < (ETYPES[e.ty].r + .2) ** 2) {
          ...
          const bd2 = ETYPES[e.ty].boss ? b.dmg * .85 : b.dmg; // bosses shrug off 15% of bullet damage
          if (!b.ghost) { if (host) this._dmgEnemy(e, bd2, b); else { e.flash = .12; (this._hitQ = this._hitQ || []).push([e.id, +bd2.toFixed(1)]); } }
          if (b.pierce > 0) { b.pierce--; b.dmg *= .85; } else b.life = 0; // pierce decays 15% per hop
          break;
        }
      }
    }
    ...
    for (const b of this.ebullets) {
      b.x += b.dx * dt; b.z += b.dz * dt; b.life -= dt;
      if (host) {
        if (!this.me.down && dist2(b.x, b.z, this.me.x, this.me.z) < .49) { this._hurt(this.me, 4.8 * this._dMul() * (this.dmgWaveMul || 1)); b.life = 0; }
        else if (this.allyOn && !this.ally.down && dist2(b.x, b.z, this.ally.x, this.ally.z) < .49) { this._dealToPlayer(this.ally, 4.8 * this._dMul() * (this.dmgWaveMul || 1)); b.life = 0; }
