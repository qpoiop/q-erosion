...
export function install(P) {
  ...
  P._structPack = function () { const a = []; for (let i = 0; i < N * N; i++) if (this.occ[i] === 1 || this.occ[i] === 2 || this.occ[i] === 5) a.push([i, this.occ[i], Math.round(this.shp[i]), +this.bld[i].toFixed(2)]); return a; }
  P._structUnpack = function (a) {
    const had = new Set();
    for (const [i, k, hp, b] of a) {
      had.add(i); if (this.occ[i] !== k) this.occ[i] = k; this.shp[i] = hp;
  ...
  P._netTick = function (dt) {
    ...
    if (this.isHost) {
      ...
      if (this.sendStateT <= 0) {
        ...
        const o = { t: 's', tm: +this.tm.toFixed(1), xp: this.xpTotal(), sc: Math.round(this.scrap), core: Math.round(this.coreHp), wv: this.wave, ph: this.phase, pt: +this.phT.toFixed(1), qn: this.spawnQ.length, gt: this.activeGate, gts: this.activeGates, bg: this._bgPaused ? 1 : 0, asc: Math.round(this.allyScrap),
